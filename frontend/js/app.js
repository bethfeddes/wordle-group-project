class WordleGame {
  ...
  async startNewGame() {
    try {
      ...
      const errorMessage = this.apiService.handleApiError(
        error,
  ...
  async submitGuess() {
    ...
    try {
      ...
      const response = await this.apiService.submitGuess(
  ...
  async handleGameWin() {
    ...
    try {
      const updatedGame = await this.apiService.getGameState(
        this.currentGame.gameId,
      );
    ...
    const attempts = this.gameBoard.currentRow;
    this.showMessage(
  ...
  async handleGameLoss() {
    ...
    this.showMessage(
      `😔 Game Over! The word was ${this.currentGame.targetWord}`,
    );
